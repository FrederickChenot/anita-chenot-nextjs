    ...
    <main className="py-20 px-6">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="space-y-12">
          {prestations.map((p, i) => (
            <div key={p.id} className={`flex flex-col ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"} gap-10 items-center`}>
              {p.img && (
                <div className="w-full md:w-2/5 rounded-lg overflow-hidden">
                  <img src={p.img} alt={p.title} className="w-full h-64 object-cover hover:scale-105 transition-transform duration-500"/>
                </div>
              )}
              <div className="flex-1">
                <h2 className="font-[var(--font-playfair)] text-2xl text-[#2C2416] mb-4 capitalize">{p.title}</h2>
